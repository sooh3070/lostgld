...
const FishingTool = () => {
  ...
  useEffect(() => {
    ...
    const filtered = stored.filter((p) => p.toolType === "fishing");
    setPresets(filtered);
  }, []);
  ...
  const handleDeletePreset = (id) => {
    ...
    setPresets(updated.filter((p) => p.toolType === "fishing"));
  };
  ...
  const handleSaveToLocalStorage = () => {
    ...
    const updatedFishingData = lumberingData.items.reduce((acc, item) => {
      acc[`4T 낚시(만생기 기준)-${item.name}`] = item.count;
      return acc;
    }, {});
    const updatedData = { ...existingData, ...updatedFishingData };
