...
const HuntingTool = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const filteredData = dataList.find(
          (activity) => activity.name === "4T 수렵(만생기 기준)"
        );
        ...
        console.error("수렵 데이터 로드 실패:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const filtered = stored.filter((p) => p.toolType === "hunting");
    setPresets(filtered);
  }, []);
  ...
  const handleDeletePreset = (id) => {
    ...
    setPresets(updated.filter((p) => p.toolType === "hunting"));
  };
  ...
  const handleSaveToLocalStorage = () => {
    ...
    const updatedHuntingData = lumberingData.items.reduce((acc, item) => {
      acc[`4T 수렵(만생기 기준)-${item.name}`] = item.count;
      return acc;
    }, {});
    const updatedData = { ...existingData, ...updatedHuntingData };
